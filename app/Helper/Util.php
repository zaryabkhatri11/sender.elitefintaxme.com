...
class Util
{
    ...
    public static function getFromFirebaseRC()
    {
        $path = base_path('.firebaserc');

        if (!file_exists($path)) {
            return null;
        }
    
        $firebaserc = file_get_contents($path);
        $firebaserc = json_decode($firebaserc);
    
        return $firebaserc->firebase ?? null;
    }
}